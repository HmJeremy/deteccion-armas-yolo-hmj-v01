#!/usr/bin/env python3
"""Punto de entrada único del proyecto: detecta armas de fuego en una
imagen, un video, la webcam o una cámara de seguridad IP.

La fuente se interpreta automáticamente:
    - un índice numérico ("0", "1", ...)      -> webcam local
    - una ruta o URL que termina en .jpg/.png/.jpeg/.bmp -> imagen fija
    - cualquier otra ruta o URL (.mp4, rtsp://, http://<ip>:8080/video, etc.)
      -> stream de video, tratado frame a frame en tiempo real
...
Requiere los pesos entrenados en `models/` (no incluidos en el repo por
tamaño, ver models/README.md) o rutas explícitas con --pesos/--config.
"""
