...
Envuelve ``cv2.dnn`` para cargar un modelo Darknet (.cfg + .weights) y
correr inferencia sobre imágenes o frames de video, aplicando un umbral
de confianza real antes de non-maxima suppression (a diferencia de los
scripts originales de la tesis, que pasaban todas las detecciones con
confianza > 0 directo a NMS).
"""
...
class WeaponDetector:
    ...
    classes:
        ...
        Por defecto ``["Arma"]`` (modelo de una sola clase de la tesis).
    input_size:
        Tamaño (ancho, alto) del blob de entrada. YOLOv3/v4 estándar usa 416.
    confidence_threshold:
        Confianza mínima para considerar una detección válida. Los scripts
        originales usaban 0 (sin filtro real); acá se filtra antes de NMS.
    nms_threshold:
        Umbral de IoU para non-maxima suppression.
    backend, target:
        Backend/target de ``cv2.dnn`` (por defecto CPU). Pasa
        ``cv2.dnn.DNN_BACKEND_CUDA`` / ``cv2.dnn.DNN_TARGET_CUDA`` si
        compilaste OpenCV con soporte CUDA para acelerar la inferencia.
    """
    ...
    def __init__(
        ...
        if not weights_path.exists():
            raise FileNotFoundError(
                ...
                "Ver models/README.md para dónde descargarlo."
            )
    ...
    def detect(self, image: np.ndarray) -> list[Detection]:
        ...
        las detecciones que sobreviven al umbral de confianza + NMS."""
