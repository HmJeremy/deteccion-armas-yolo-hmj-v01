...
En la versión original de la tesis tenía tres scripts sueltos para esto
(``pruebas_map.py``, ``pruebas_map2.py``, ``pruebas_map3.py``), cada uno una
variación a medio terminar del mismo cálculo. Este archivo los junta todos
en una sola función que calcula IoU, matriz de confusión, precisión, recall
y exactitud.

Espera una carpeta con pares ``imagen.jpg`` + ``imagen.txt`` en formato YOLO
(``class x_center y_center width height``, normalizado 0-1) — el mismo
formato que usa ``darknet/data/obj``.
"""
...
def evaluate_dataset(
    ...
    compara contra su anotación ``.txt`` (formato YOLO), emparejando cajas
    por IoU. Devuelve un :class:`EvalResult` con TP/FP/FN ya sumados."""
